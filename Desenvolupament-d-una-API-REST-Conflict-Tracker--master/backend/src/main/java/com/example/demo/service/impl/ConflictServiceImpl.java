...
public class ConflictServiceImpl implements ConflictService {
    ...
    public List<ConflictDTO> findByStatus(Status status) {
        return conflictRepository.findByStatus(status).stream().map(ConflictMapper::toDTO).collect(Collectors.toList());
    }
    ...
    public List<ConflictDTO> findByCountryCode(String countryCode) {
        return conflictRepository.findByCountryCode(countryCode).stream().map(ConflictMapper::toDTO).collect(Collectors.toList());
    }
}
