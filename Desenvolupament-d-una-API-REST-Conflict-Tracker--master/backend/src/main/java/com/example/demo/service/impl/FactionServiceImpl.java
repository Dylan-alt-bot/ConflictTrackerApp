...
public class FactionServiceImpl implements FactionService {
    ...
    public FactionServiceImpl(FactionRepository factionRepository, ConflictRepository conflictRepository, CountryRepository countryRepository) {
    ...
    public FactionDTO findById(Long id) {
        Faction faction = factionRepository.findById(id).orElseThrow(() -> new RuntimeException("Faction not found"));
        return FactionMapper.toDTO(faction);
    }

    @Override
    public List<FactionDTO> findByConflict(Long conflictId) {
        return factionRepository.findByConflictId(conflictId).stream().map(FactionMapper::toDTO).collect(Collectors.toList());
    }
    ...
    public FactionDTO create(CreateFactionDTO dto) {
        ...
        Conflict conflict = conflictRepository.findById(dto.getConflictId()).orElseThrow(() -> new RuntimeException("Conflict not found"));
